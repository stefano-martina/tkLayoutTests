void compareOuter(string oneFileName, string twoFileName) {
  TFile* oneFile = TFile::Open(oneFileName.c_str());
  TFile* twoFile = TFile::Open(twoFileName.c_str());

  TCanvas* oneCanvas = (TCanvas*) oneFile->Get("matTrack000");
  TCanvas* twoCanvas = (TCanvas*) twoFile->Get("matTrack000");
  ...
  TPad* onePad1 = (TPad*) oneCanvas->GetPrimitive("materialInTrackingVolumeouter_1");
  TPad* twoPad1 = (TPad*) twoCanvas->GetPrimitive("materialInTrackingVolumeouter_1");
  ...
  std::cerr << "xMax first = " << oneProfile1->GetXaxis()->GetXmax() << std::endl;
  std::cerr << "xMax second = " << twoProfile1->GetXaxis()->GetXmax() << std::endl;
  ...
  TPad* onePad2 = (TPad*) oneCanvas->GetPrimitive("materialInTrackingVolumeouter_2");
  TPad* twoPad2 = (TPad*) twoCanvas->GetPrimitive("materialInTrackingVolumeouter_2");
  ...
  TCanvas* out = new TCanvas("outOuter", "Compare");
  out->Divide(2,1);
  ...
  TLegend* leg1 = new TLegend(0.675061, 0.757787, 0.900139, 0.900107);
  ...
  TLegend* leg2 = new TLegend(0.675061, 0.757787, 0.900139, 0.900107);
