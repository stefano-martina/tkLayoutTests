void macroLayer3() {
  TFile* file = TFile::Open("origL3.root");
  ...
  double w = 1400;
  double h = 900;
  TCanvas* outCanv = new TCanvas("out", "Layer 3", w, h);
  ...
  //layer cylinder
  TF1* funcL = new TF1("funcL", "(0.1*36*(TMath::E()^x+TMath::E()^(-x)))/(2*TMath::Pi()*119.538*0.1286*2)", 0., 4.);
  funcL->SetLineColor(2);
  funcL->Draw("same");

  //exiting cylinder
  TF1* funcE = new TF1("funcE", "(0.1*36*(TMath::E()^x+TMath::E()^(-x)))/(2*TMath::Pi()*189.368*0.1286*2)", 0., 4.);
  funcE->SetLineColor(3);
  funcE->Draw("same");
  
  //disks
  TF1* funcV3 = new TF1("funcV3", "(0.1*36*(TMath::E()^(2*x)+1))/(TMath::Pi()*(166.368+114.538)*0.1286*(TMath::E()^(2*x)-1))", 0., 4.);
  ...
  for(int bin=1; bin<=origProf->GetNbinsX(); ++bin) {
    ...
    } else if((eta >= etas[4]) && (eta <= etas[5])) {  //exiting
